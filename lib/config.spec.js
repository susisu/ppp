...
import { load, getFields, getWrapSize } from "./config.js";
...
describe("load", () => {
  ...
  it("throws an error if the file content cannot be parsed", async () => {
    ...
    await expect(load([brokenFile])).rejects.toThrowError(
      "Failed to load config: Invalid config format"
    );
  });
  ...
  it("throws an error if the content of the file content is not an object", async () => {
    ...
    await expect(load([invalidFile])).rejects.toThrowError(
      "Failed to load config: Invalid config format"
    );
  });
  ...
  it("throws an error if the file cannot be read for some reason", async () => {
    ...
    await expect(load([forbiddenFile])).rejects.toThrowError("Failed to load config");
  });
...
describe("getFields", () => {
  ...
  it("throws an error if 'fields' is not an array", () => {
    ...
    expect(() => getFields(conf)).toThrowError(
      "Failed to load config value: 'fields' must be a list of field names"
    );
  });
...
describe("getWrapSize", () => {
  it("returns 'wrap' of the config if it is a number", () => {
    const conf = {
      wrap: 80,
    };
  ...
  it("returns Infinity if it is negative", () => {
    const conf = {
      wrap: -42,
    };
    expect(getWrapSize(conf)).toBe(Infinity);
  });
  ...
  it("throws an error if 'wrap' is not a number", () => {
    const conf = {
      wrap: null,
    };
    expect(() => getWrapSize(conf)).toThrowError(
      "Failed to load config value: 'wrap' must be a number"
    );
  });

  it("throws an error if 'wrap' is NaN", () => {
    const conf = {
      wrap: NaN,
    };
    expect(() => getWrapSize(conf)).toThrowError(
