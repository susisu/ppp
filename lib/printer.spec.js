...
import { validateFields, print } from "./printer.js";

describe("validateFields", () => {
  it("does nothing if the input contains only valid fields", () => {
    expect(() => validateFields(["name", "version"])).not.toThrow();
  });

  it("throws an error if the input contains some invalid fields", () => {
    expect(() => validateFields(["name", "xxx"])).toThrow("Unknown field 'xxx'");
  });
});
...
describe("print", () => {
  ...
  it("throws error if unknown field is specified", async () => {
    const pkg = {};
    const promise = print(pkg, ["unknown"], opts, {});
