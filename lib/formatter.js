...
function colorLinkAndEmail(msg) {
  // need better regular expressions?
  return msg.replace(/https?:\/\/[0-9A-Za-z\-_./#&?=+%~]+/g, colors.link)
    .replace(/[0-9A-Za-z\-_][0-9A-Za-z\-_.]*@[0-9A-Za-z\-_.]+/g, colors.link);
}
...
const fields = {
  ...
  maintainers(val, pkg) {
    return val.map(colorLinkAndEmail).join("\n");
  },
  repository(val, pkg) {
    return typeof val["url"] === "string" ? colorLinkAndEmail(val["url"])
      : typeof val === "string" ? colorLinkAndEmail(val)
      : null;
  },
  tags(val, pkg) {
