...
export function items(items, wrapSize, indentSize) {
  ...
  const msgSize = wrapSize !== null ? wrapSize - (labelSize + " : ".length) : null;
...
export const fields = {
  ...
  installed: val => {
    const localVersion = val.local !== null ? colors.version(val.local) : "(not installed)";
    const globalVersion = val.global !== null ? colors.version(val.global) : "(not installed)";
