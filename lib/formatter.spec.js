...
describe("items", () => {
  it("prints items as a list", () => {
    ...
    const text = formatter.items(items, Infinity, 2);
    expect(text).toBe(
      // prettier-ignore
      [
          "Foo     : foo",
  ...
  it("wraps long message if wrapSize is not Infinity", () => {
    const items = [
      {
        label: "Long",
        msg:
  ...
  it("does not wrap long message if wrapSize is Infinity", () => {
    const items = [
      {
        label: "Long",
        msg:
...
describe("fields", () => {
    ...
    it("prints installed versions", () => {
      {
        ...
        const msg = formatter.fields.installed(val);
        expect(strip(msg)).toBe("local: 1.0.0, global: 3.1.4");
      }
      {
        const val = {
          local: undefined,
        ...
        const msg = formatter.fields.installed(val);
        expect(strip(msg)).toBe("local: (not installed), global: (not installed)");
      }
    });
  });
  ...
  describe("people", () => {
    it("prints people", () => {
      ...
      expect(strip(msg)).toBe(
        ["Foo <foo@example.com>", "Bar <bar@example.com> (https://example.com/bar)"].join("\n")
      );
    });
  });
  ...
  describe("wordList", () => {
    it("prints list of words", () => {
      const val = ["foo", "bar"];
      const msg = formatter.fields.wordList(val);
